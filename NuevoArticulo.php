...
$band = 0 ; 
$bd = new Conexion();
$registros = $bd->query("SELECT `CveArt` FROM `articulos` WHERE `Status` = 1");
...
if($band == 0 ){
  $bd->query("INSERT INTO `articulos`(`Status`) VALUES (1)")or die ($bd->error);
  $registros = $bd->query("SELECT `CveArt` FROM `articulos` WHERE `Status` = 1");
  if ($registros->num_rows > 0 ){
      while($reg = mysqli_fetch_array($registros)){
        $cve = $reg['CveArt'];
  ...
  <div class="row content">
    ...
    <div class="col-sm-8 text-left"> 
      ...
      <div class="card card-login mx-auto mt-5">
        <div class="card-header bg-primary text-white"><h2>Registro de Articulos</h2></div>
          <div class="card-body">
...
<!-- Modal Para saber si quiere regresar o no-->
<div class="modal fade" id="QuestionFRM">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <a href="vistaArticulo.php" class="btn btn-success">Si</a>
...
<script src = "js/controllerArticulo.js"></script>
</body>
</html>